...
export class VendorManagementComponent implements OnInit {
  ...
  ExecelSerchBody = {};
  selectedModel: VendorManagementView;
  ...
  constructor(private cdr: ChangeDetectorRef ,    
    ...
    getMasterData(filter:string){
      ...
      switch(filter){
        case 'regions':
          this.vendorservice.getRegion().then(regions=>{
    ...
    this.dataTableModel = {
      ...
      loadData: async (params: DataLoadParams, settings) => {
        try {
          ...
          const res = await this.vendorservice
          // .getAllData(params,requestBody);
          .getAllVendor(params,requestBody);
          ...
          const viewModels = await Promise.all(res.data?.content.map((x: any) => VendorManagementModelMapper.mapToViewModel(x,null)));
  ...
  downloadTemplate() {
    const url = this.vendorservice.getTemplateDownloadUrl();
  ...
  showConfirmation(status: string, model: VendorManagementView) {
    this.modalService.show({
      ...
      okCallback: async () => {
        this.loading = true;
        try {
          let result: ApiResponse<VendorManagementView>;
          if (status === 'Active') {
            result = await this.vendorservice.activate(model.vendorId);
          } else {
            result = await this.vendorservice.deactivate(model.vendorId);
          }
  ...
  onDeleteAction(model: VendorManagementView) {
    this.modalService.show({
      title: 'Delete User',
      ...
      try {
        let result: ApiResponse<VendorManagementView>;
          result = await this.vendorservice.delete(model.vendorId);
        if (ApiErrorUtil.isError(result)) {
          this.alertService.show(ApiErrorUtil.errorMessage(result), AlertType.Critical);
        } else {
          this.alertService.show(result?.message?.messageDescription );
