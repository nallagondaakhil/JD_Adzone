...
export class DealerOrderComponent implements OnInit {
  ...
  constructor( private mockData: MockDataService, private cdr: ChangeDetectorRef , 
    private service : CategoryCountService,
  ...
  getMasterData(filter:string){
    ...
    switch(filter){
      case 'marketnew':
        console.log("madhu");
        this.service.getMarket().then(markets=>{

          markets.forEach(marketnew=>{
            options.push(marketnew);
          });
        });
        break;
  ...
  initDataTable() {
    this.dataTableModel = {
      options: {
        ...
        exportExcel: false,
        hideSearchBar: true,
        searchPlaceHolder:'Search For RACF ID,  Dealer ID',
        filters:[
          // {
          //   field: 'SupplierIds',
          //   type: 'supplier',
          ...
          // },
          {
            ...
            type: 'marketnew',
            placeholder: 'Market',
            selectOptions: this.getMasterData('marketnew')
          },
          {
            field: 'dealers',
            type: 'dealer',
        ...
        exportClicked: () => {
          this.service.initiateDealerOrderExportExcel(this.ExecelSerchBody,this.ExecelFilterBody).then((res: any) => {
      ...
      loadData: async (params: DataLoadParams, settings) => {
        try {
         ...
         // const res = "";
          ...
          .getAllDealer(params,requestBody);
  ...
  downloadTemplate() {
    const url = this.service.getDealerOrderTemplateDownloadUrl();
    this.alertService.show("File downloaded successfully. Please check after some time in your browser download section.");
    DownloadUtil.downloadFile(this.http, url, 'dealerorderreport.xlsx' , 'dealer_order_report',this.ExecelSerchBody,this.ExecelFilterBody);
  }
