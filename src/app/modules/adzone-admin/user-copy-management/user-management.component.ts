...
@Component({
  selector: 'jd-vendor-management',
  templateUrl: './user-management.component.html',
  styleUrls: ['./user-management.component.scss']
})
export class VendorManagementCopyComponent implements OnInit {
  showAddModal = false;
  ...
  constructor(private cdr: ChangeDetectorRef ,    
    ...
    async checkPermission(){
      await this.rolepermission.getPermissionForModule(this.user,"User Management");
      this.permissionView = await this.rolepermission.getRoleById("view_user");
      console.log(this.permissionView);
      this.permissionAdd = await this.rolepermission.getRoleById("create_user");
      this.permissionEdit = await this.rolepermission.getRoleById("edit_user");
      this.permissionActivate = await this.rolepermission.getRoleById("activate_user");
      this.permissionDeactivate = await this.rolepermission.getRoleById("deactivate_user");
      this.permissionBulkUpload = await this.rolepermission.getRoleById("bulk upload_user");
      this.permissionDelete = await this.rolepermission.getRoleById("delete_user");
    }
    ...
    this.dataTableModel = {
      ...
      loadData: async (params: DataLoadParams, settings) => {
        try {
          ...
          if(!this.filterApplied || this.searchClicked) {
            console.log('filetClicked')
            // var getRacfIdParams: any = {};
            // getRacfIdParams['searchKey'] = params.searchKey;
            const getAllUserRacfId = await this.userservicemangement.getAllUserRacfId(params, requestBody);
            // this.getRacfIds = getAllUserRacfId?.data?.map((x:any)=>x)
            getAllUserRacfId.data.forEach(item=>{
              this.getRacfIds.push({'racfId': item})
            })
            // getAllUserRacfId.data.map((x: any) => {
            //   this.getRacfIds.push({
            //     'racfId': x
            //   });
            // console.log("racIfId's",this.getRacfIds)
            // });
          }
  ...
  downloadTemplate() {
    ...
    DownloadUtil.downloadFile(this.http, url, 'userreport.xlsx' , 'user-management',this.ExecelSerchBody,this.ExecelFilterBody);
  }
      ...
      okCallback: async () => {
        ...
        try {
          ...
          if (ApiErrorUtil.isError(result)) {
            ...
            this.alertService.show(ApiErrorUtil.errorMessage(error) || 'Failed to update User', AlertType.Critical);
        }
        this.loading = false;
