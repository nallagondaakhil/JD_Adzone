...
@Component({
  selector: 'jd-add-edit-copy-vendor',
  templateUrl: './add-edit-vendor.component.html',
  styleUrls: ['./add-edit-vendor.component.scss']
})
export class AddEditVendorCopyComponent implements OnInit {
  title='';
  buttonText='';
  resetbutton='';
  loading = false;
  ...
  async ngOnInit() {
    ...
    this.title = this.model ? 'Update User' : 'Add User';
  ...
  onCreateUpdate(status: string,model:any,result:any) {
    ...
    return new Promise((resolve) => {
      this.modalService.show({
        title: status === 'create' ? 'Create User' : 'Update User',
        message: `Are you sure you want to ${status === 'create' ? 'Create' : 'Update'} this user?`,
        okText: 'Yes',
        okCallback: async () => {
          if(status === 'create')
  ...
  async onsave(){
    ...
    try {
      ...
      if(this.isEdit){
        ...
        this.alertService.show(ApiErrorUtil.errorMessage(error) || 'Failed to add user ', AlertType.Critical);
