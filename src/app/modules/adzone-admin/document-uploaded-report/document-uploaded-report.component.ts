...
@Component({
  selector: 'jd-document-uploaded-report',
  templateUrl: './document-uploaded-report.component.html',
  styleUrls: ['./document-uploaded-report.component.scss']
})
export class DocumentUploadedReportComponent implements OnInit {
  ...
  constructor( private mockData: MockDataService, private cdr: ChangeDetectorRef ,
    private service : CategoryCountService,private roleService :RoleService,private alertService:AlertsService,private http:HttpClient) { }
  ...
  initDataTable() {
    this.dataTableModel = {
      options: {
        ...
        exportExcel: false,
        hideSearchBar: true,
        searchPlaceHolder:'Search For Year, Month, Document Uploaded by',
        filters:[
          {
            field: 'roleNames',
            type: 'roles',
        ...
        exportClicked: () => {
          this.service.initiateExportDocumentUploadExcel(this.ExecelSerchBody,this.ExecelFilterBody).then((res: any) => {
      ...
      loadData: async (params: DataLoadParams, settings) => {
        try {
         ...
         // const res = "";
           .getAllUploaded(params,requestBody);
          // .getAllData(10,10);
          ...
          // viewModels = await Promise.all(res.data?.map((x: any) => CategoryCountReportModelMapper.mapToViewModel(x)));
  ...
  downloadTemplate() {
    const url = this.service.getUploadTemplateDownloadUrl();
    this.alertService.show("File downloaded successfully. Please check after some time in your browser download section.");

    DownloadUtil.downloadFile(this.http, url, 'documentupload.xlsx' , 'document_uploaded',this.ExecelSerchBody,this.ExecelFilterBody);
  }
